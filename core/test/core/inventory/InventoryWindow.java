...
import com.mygdx.game.world.entity.item.Inventory;
import com.mygdx.game.world.entity.item.WornItem;
import com.mygdx.game.world.enums.item.EquipmentSlot;

public class InventoryWindow extends Window implements InventorySubject, InventorySlotObserver {

    private final static int _numSlots = 50;
    private int _lengthSlotRow = 10;

    private final int _slotWidth = 64;
    private final int _slotHeight = 64;
    ...
    private Label _APValLabel, _DPValLabel;
    private int _DPVal, _APVal = 0;
    ...
    public InventoryWindow(String title, Skin skin, Hero hero) {
        super(title, skin, "solidbackground");
        this.hero = hero;

        _observers = new Array<InventoryObserver>();

        _dragAndDrop = new MyDragAndDrop();
        ...
        InventorySlot leftArmSlot = new InventorySlot(EquipmentSlot.Weapon,
            ...
            inventorySlot.addListener(new ClickListener() {
                                          ...
                                          public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                                              ...
                                              if (getTapCount() == 2) {
                                                  ...
                                                  if (slot.hasItem()) {
                                                      WornItem item = slot.getTopInventoryItem();
                                                      if (item.isConsumable()) {
                                                          String itemInfo = item.getName() + ", " + item.getCost();
                                                          InventoryWindow.this.notify(itemInfo, InventoryObserver.InventoryEvent.ITEM_CONSUMED);
        ...
        populateInventory(_inventorySlotTable, hero.getInventory(), _dragAndDrop, "empty", false);
    }
...
*/
    public static void populateInventory(Table targetTable, Inventory inventory, MyDragAndDrop draganddrop, String defaultName, boolean disableNonDefaultItems) {
        clearInventoryItems(targetTable);

        Array<Cell> cells = targetTable.getCells();
        for (int i = 0; i < inventory.getItems().length; i++) {
            WornItem item = inventory.getItems()[i];

            if (item != null) {
                if (item.getDrawable() == null) {
                    item.setDrawable(new TextureRegionDrawable((Texture) Utility.assetManager.get(item.getIconPath())));
                }
        ...
        for(int i = 0; i < cells.size; i++){
            ...
            if( numItems > 0 ){
                items.add(new InventoryItemLocation(
                        ...
                        //inventorySlot.getTopInventoryItem().getItemTypeID().toString(),   todo
                        numItems,
                        inventorySlot.getTopInventoryItem().getName()));
            }
        }
        ...
        for(int i = 0; i < cells.size; i++){
            ...
            if( numItems > 0 ){
                ...
                items.add(new InventoryItemLocation(
                        i,
                        *//*inventorySlot.getTopInventoryItem().getItemTypeID().toString(),*//* //todo
                        inventorySlot.getTopInventoryItem().getName().toString(),
                        numItems,
                        name));
            }
        }
...
*/
    ...
    public void addEntityToInventory(WornItem entity, String itemName) {
        Array<Cell> sourceCells = _inventorySlotTable.getCells();
        int index = 0;
        ...
        for (; index < sourceCells.size; index++) {
            ...
            int numItems = inventorySlot.getNumItems();
            if (numItems == 0) {
                WornItem inventoryItem = new WornItem();
                /*InventoryItemFactory.getInstance().getInventoryItem(ItemTypeID.valueOf(entity.getEntityConfig().getItemTypeID())*/
    ...
    public void onNotify(InventorySlot slot, SlotEvent event) {
        switch (event) {
            case ADDED_ITEM:
                WornItem addItem = slot.getTopInventoryItem();
    ...
    public boolean removeActor(Actor actor, boolean unfocus) {
        if (actor instanceof WornItem) {
            return super.removeActor(actor, false);
        }
