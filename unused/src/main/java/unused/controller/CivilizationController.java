...
import unused.world.entity.civilization.Civilization;
import unused.world.entity.civilization.ForeignPolitics;
import unused.world.entity.game.Game;
import unused.world.enums.adventure.Direction;
import herotycoon.cartezza.mapgenerator.generator.utils.Randomizer;
...
public class CivilizationController {
    ...
    public void processBordersExpansion(final Game game, final Civilization civilization) {
        ...
        for (int i = 0; i < regionsLayer[0].length; i++) {
            for (int j = 0; j < regionsLayer.length; j++) {
        ...
        while (points.iterator().hasNext()) {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (updatedPoint.x >= 0
                    ...
                    if (Randomizer.range(0, 10) > hostilityLayer[updatedPoint.y][updatedPoint.x]
                            && regionsLayer[updatedPoint.y][updatedPoint.x] == 0
                            && hostilityLayer[updatedPoint.y][updatedPoint.x] < 10) {
                        regionsLayer[updatedPoint.y][updatedPoint.x] = regionId;
                        count++;
                    }
                }
            }
        ...
        for (Civilization civ : worldCivs) {
            ...
            for (Point point : points) {
                ...
                for (Direction direction : Direction.values()) {
                    ...
                    if (updatedPoint.x >= 0
                            && updatedPoint.y >= 0
                            && updatedPoint.x < regionsLayer[0].length
                            && updatedPoint.y < regionsLayer.length
                            && regionsLayer[updatedPoint.y][updatedPoint.x] == civ.getRegionId()) {
                        count++;
                    }
                }
            }
